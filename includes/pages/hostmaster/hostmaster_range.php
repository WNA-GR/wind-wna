...
class hostmaster_range {
	...
	function form_range() {
		...
		$form_range->db_data('ip_ranges.ip_start, ip_ranges.ip_end, ip_ranges.info, ip_ranges.status');#removed: ip_ranges.nodeid #bug
	...
	function output() {
		...
		if(get('action') === "delete")
		{
			...
			if ($ret) {
				#@#
				log_admin_action(get('iprange'),"range#".get('iprange'), "delete_success");
	...
	function output_onpost_form_range() {
		...
		if ($ret) {
			log_admin_action(get('iprange'),"range#".get('iprange'), "edit_success");
