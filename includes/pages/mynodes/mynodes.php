...
class mynodes {
	...
	function form_node() {
		...
		$form_node->db_data('nodes.name, nodes.declared_status, nodes.area_id, nodes.community_id ,nodes.com_wind_id, nodes.latitude, nodes.longitude, nodes.elevation, nodes.info,nodes.freeifs, nodes.url,  nodes.internetaccess, nodes.internetprovider, nodes.mikrotik_license_software_id, nodes.autonomy_minutes,  nodes.system_identity'.($this->has_owner_access() || get('node') == 'add'?', users_nodes.user_id, users_nodes.user_id':''));
...
#@#
	function table_nodesettingschanges() {
		...
		/*foreach( (array) $table_nodesettingschanges->data as $key => $value) {
			if ($key != 0) {
				if ($table_nodesettingschanges->data[$key]['ip'])
					$table_nodesettingschanges->data[$key]['ip'] = long2ip($table_services->data[$key]['ip']);
				$table_nodesettingschanges->info['EDIT'][$key] = makelink(array("page" => "mynodes", "subpage" => "services", "node" => intval(get('node')), "service" => $table_services->data[$key]['id']));
			}
		}*/
		#$table_nodesettingschanges->info['EDIT_COLUMN'] = 'title';
		#$table_nodesettingschanges->db_data_translate('nodes_services__status');
		#$table_nodesettingschanges->db_data_multichoice('nodes_services', 'id');
		#$table_nodesettingschanges->info['MULTICHOICE_LABEL'] = 'delete';
		#$table_nodesettingschanges->db_data_remove('id','nodes__id');
...
#@#
	...
	function output() {
		...
		if (get('action') == 'delete') {
			...
			if (get('node') != 'add') {
				...
				$this->tpl['link_nodesettingschanges_add'] = makelink(array('page' => 'mynodes', 'subpage' => 'nodesettingschanges', 'node' => get('node'), 'nodesettingschanges' => 'add'));
	...
	function output_onpost_form_node() {
		...
		if ($ret) {
			log_admin_action($nodes,"node#".get('node'), (get('node') == 'add'?'insert':'edit').'_success');#@#
			#@#
						
			#$nowtime = $this->date_now(); 
			#$thisnode = get('node_id');
			#$query = $db->query("UPDATE `nodes` SET
