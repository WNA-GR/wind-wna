...
class menu {
	...
	function output() {
		...
		if ($main->userdata->logged) {
			...
			#@#
			$this->tpl['home_node'] = $db->get('nodes.id, nodes.name', 'users INNER JOIN users_nodes ON users.id = users_nodes.user_id AND users.home_node = users_nodes.node_id Inner Join nodes ON nodes.id = users_nodes.node_id', "users_nodes.user_id ='".$main->userdata->user."'");			
			foreach( (array) $this->tpl['home_node'] as $key => $value) {
			$this->tpl['home_node'][$key]['url'] = makelink(array("page" => "mynodes", "node" => $this->tpl['home_node'][$key]['id']));
			$this->tpl['home_node'][$key]['url_view'] = makelink(array("page" => "nodes", "node" => $this->tpl['home_node'][$key]['id']));
			}#@#
			...
			if ($main->userdata->privileges['admin'] === TRUE) {
				...
				$this->tpl['link_admin_nodechangeslog'] = makelink(array('page' => 'admin', 'subpage' => 'nodechangeslog'));
			}
			if ($main->userdata->privileges['cadmin'] === TRUE) {
				$this->tpl['is_admin'] = TRUE;
				$this->tpl['link_admin_areas'] = makelink(array('page' => 'admin', 'subpage' => 'areas'));
		...
		$this->tpl['link_nodechangeslog'] = makelink(array("page" => "nodechangeslog"));
