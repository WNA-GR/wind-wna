	...
	if (GBrowserIsCompatible()) {
		...
		GEvent.addListener(map, 'click', function(overlay, point) {
			if (overlay) {
				...
				marker_point = point;
				#var html = '<div style="padding-right: 15px; white-space: nowrap; text-align:left; font-size:10px;">{/literal}{$lang.db.nodes__latitude}{literal}: ' + (Math.round(marker_point.k * 1000000)/1000000) + '<br />' + '{/literal}{$lang.db.nodes__longitude}{literal}: ' + (Math.round(marker_point.a * 1000000)/1000000) + '<br /><br />' + '<a href="" onclick="window.opener.pickup_value(window.opener.document.{/literal}{$object_lat|escape:"quotes"}{literal}, Math.round(marker_point.k * 1000000) / 1000000); window.opener.pickup_value(window.opener.document.{/literal}{$object_lon|escape:"quotes"}{literal}, Math.round(marker_point.a * 1000000)/1000000); window.close(); return false;">{/literal}{$lang.select_the_coordinates}{literal}</a></div>';
var html = '<div style="padding-right: 15px; white-space: nowrap; text-align:left; font-size:10px;">{/literal}{$lang.db.nodes__latitude}{literal}: ' + x + '<br />' + '{/literal}{$lang.db.nodes__longitude}{literal}: ' + y + '<br /><br />' + '<a href="" onclick="window.opener.pickup_value(window.opener.document.{/literal}{$object_lat|escape:"quotes"}{literal}, x; window.opener.pickup_value(window.opener.document.{/literal}{$object_lon|escape:"quotes"}{literal}, y; window.close(); return false;">{/literal}{$lang.select_the_coordinates}{literal}</a></div>';
