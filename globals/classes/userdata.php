...
function mybb_login($username, $password_md5) {
	...
	// Get user info
	$query = $db->query("SELECT username,password,salt,usergroup FROM {$table_prefix}users WHERE username='".mysql_escape_string($username)."'");
...
function mybb_sync_db($username, $password_md5) {
	...
	// Get MyBB User info
	$query = $db->query("SELECT username,usergroup,email FROM {$table_prefix}users WHERE username='".mysql_escape_string($username)."'");
	$data = mysql_fetch_assoc($query);
